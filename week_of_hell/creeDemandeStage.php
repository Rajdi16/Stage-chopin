...
if (!isset($_SESSION['id'])) {
    echo "Vous devez être connecté pour faire une demande de stage.";
    exit();
}
...
if (isset($_POST['cree'])) {
    if (!empty($_POST['dateDeb']) && !empty($_POST['dateFin']) && !empty($_POST['description']) && !empty($_POST['etat'])) {
        ...
        $stmt = $conn->prepare("INSERT INTO demandestage (`dateDeb`, `dateFin`, `description`, `etat`, `etudiant_Id`) VALUES (:dateDeb, :dateFin, :description, :etat, :etudiant_Id)");
